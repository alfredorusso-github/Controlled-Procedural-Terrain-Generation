...
public class TerrainScript : MonoBehaviour{
    ...
    private Vector2 getNewLocation(Vector2 location, Vector2 prevLocation, float aHeight, List<Vector2> roadPoints, bool start = true){
    ...
    private bool chcekPoint(Vector2 point, Vector2 prevLocation){
        ...
        for(int i=0; i<=20; i++){
            if(point + Vector2.up * i == prevLocation || point + Vector2.down * i == prevLocation || point + Vector2.right * i == prevLocation || point + Vector2.left * i == prevLocation){
                return false;
            }
        }
    ...
    private void prova(){
        ...
        for(int i=0; i<x; i++){
            for(int j=0; j<y; j++){
                if(heightmap[j, i] > ah+.05 && squaredSteepness(new Vector2(i, j), 8) && (i >= 0 && i <= (x-1)) && (j >= 0 && j <= (y-1))){
                    heightmap[j, i] = 1.0f;
                }
            }
        }
