...
public class CoastlineAgent : MonoBehaviour
{
    ...
    private IEnumerator Action()
    {
        while (agents.Count != 0)
        {
            ...
            // Store the center of the island (starting point)
            if (_firstTime)
            {
                _center = location;
                _firstTime = false;
                Debug.Log("Map Center: " + location);
            }
    ...
    {
        return new Vector2((x - 1) - attractor.x, (y - 1) - attractor.y);
    }
    ...
    private float scorePoint(Vector2 point, Agent agent, Vector2 attractor, Vector2 repulsor)
    {
        float result = Mathf.Pow(Vector2.Distance(point, repulsor), 2.0f) -
                       ...
                       (3 * Mathf.Pow(getClosestDistance(point, agent), 2.0f));
        return result;
    }
